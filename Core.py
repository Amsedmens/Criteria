...
class CoreNeuralNetwork:
    ...
    def __init__(self):
        ...
        (self.X_train, self.X_test, self.y_train, self.y_test) = train_test_split(np.zeros((60000, 28, 28, 1)), np.zeros((10000, 28, 28, 1)), test_size=0.2, random_state=42)
    ...
    def train_lstm(self):
        ...
        lstm_model.fit(np.zeros((100, 10, 1)), np.zeros((100, 1)), epochs=10, batch_size=32)
...
if __name__ == '__main__':
    ...
    core.train_lstm()
    core.train_convnet()
    prediction = core.predict(np.zeros((1, 28, 28, 1)))
    print(prediction)
